...
def run_parking_dwell_state_multi_registered(
    ...
    # 8) save the mask
        # --- [NOUVEAU] Exports de mask pour le post-traitement ---

    # 1) Mask binaire en image (H,W), issu du seuil sur score_img normalisé (0/1)
    #    Si tu préfères un seuil ABSOLU sur score_brut, remplace "mask_img" ci-dessous
    #    par un resize de (score_grid_brut >= score_min_abs) avec INTER_NEAREST.
    mask_img = (score_img >= float(thr)).astype(np.uint8)  # (H,W) 0/1
    cv2.imwrite(f"{out_prefix}_mask_binary.png", (mask_img * 255).astype(np.uint8))

    # 2) Mask sur la grille (hG,wG) — on repart de score_grid (grille normalisée) :
    mask_grid = (score_grid >= float(thr)).astype(np.uint8)  # (hG,wG)
    np.save(f"{out_prefix}_mask_grid.npy", mask_grid)

    # 3) Polygones (contours) en coords pixel de l'image ref
    cnts, _ = cv2.findContours((mask_img*255).astype(np.uint8),
                               cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    polygons = []
    for c in cnts:
        if len(c) < 3:  # ignorer les micro-tâches
            continue
        poly = c.reshape(-1, 2).astype(float).tolist()  # [[x,y],...]
        polygons.append({"points": poly})

    import json
    with open(f"{out_prefix}_mask_polygons.json", "w", encoding="utf-8") as f:
        json.dump({
            "image_size": [int(W_img), int(H_img)],   # [width, height]
            "cell": int(cell),
            "polygons": polygons,                     # en coordonnées pixel (image ref)
        }, f, ensure_ascii=False, indent=2)

    # 4) (optionnel) Sauvegarde compacte des données utiles pour repro + stats
    np.savez_compressed(
        f"{out_prefix}_pack.npz",
        mask_img=mask_img.astype(np.uint8),
        mask_grid=mask_grid.astype(np.uint8),
        score_grid_brut=score_grid_brut.astype(np.float32),
        dwell_stop=dwell_stop.astype(np.float32),
        move_hits=move_hits.astype(np.float32),
        H_list=np.stack(H_list, axis=0),   # homographies (K,3,3)
        H_img=np.array(H_img, np.int32),
        W_img=np.array(W_img, np.int32),
        cell=np.array(cell, np.int32),
    )
    print("[OK] Saved masks (PNG, NPY, JSON) and packed NPZ.")
...
if __name__ == "__main__":
    ...
    run_parking_dwell_state_multi_registered(
        ...
        thr=0.70,   #0.9
        out_prefix="Results/parking_detection/dwell_mult/test2_thr_0.7/parking_dwell_state_MULTI_REG",
        save_reg_debug=True
    )
